/*
 * CPU-Only Rasterizer Header
 * Modified version of rasterizer.h with CUDA dependencies removed
 */
...
// CPU-only build - remove device decorators
#ifdef CPU_ONLY
    #define __host__
    #define __device__
#else
...
// Inline helper functions that work on both CPU and GPU
...
// GPU implementation (only in CUDA builds)
#ifndef CPU_ONLY
...
// Grid hierarchy functions (CPU only, no CUDA)
